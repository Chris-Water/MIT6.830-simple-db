...
public class BTreeFileDeleteTest extends SimpleDbTestBase {
    ...
    public void testStealFromLeftLeafPage() throws Exception {
        ...
        BTreeLeafPage page = BTreeUtility.createRandomLeafPage(pageId, 2, keyField, BTreeUtility.getNumTuplesPerPage(2) / 2 - 1, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE);
        ...
        assertTrue(sibling.reverseIterator().next().getField(keyField).compare(Op.LESS_THAN_OR_EQ, page.iterator().next().getField(keyField)));
    }
    ...
    public void testStealFromRightLeafPage() throws Exception {
        ...
        BTreePageId pageId = new BTreePageId(tableid, 1, BTreePageId.LEAF);
        BTreePageId siblingId = new BTreePageId(tableid, 2, BTreePageId.LEAF);
        BTreeLeafPage page = BTreeUtility.createRandomLeafPage(pageId, 2, keyField, BTreeUtility.getNumTuplesPerPage(2) / 2 - 1, 0, BTreeUtility.MAX_RAND_VALUE / 2);
        ...
        assertTrue(page.getNumTuples() == totalTuples / 2 || page.getNumTuples() == totalTuples / 2 + 1);
        assertTrue(sibling.getNumTuples() == totalTuples / 2 || sibling.getNumTuples() == totalTuples / 2 + 1);
        assertTrue(page.reverseIterator().next().getField(keyField).compare(Op.LESS_THAN_OR_EQ, sibling.iterator().next().getField(keyField)));
    }
    ...
    public void testMergeLeafPages() throws Exception {
        ...
        BTreeLeafPage leftPage = BTreeUtility.createRandomLeafPage(leftPageId, 2, keyField, BTreeUtility.getNumTuplesPerPage(2) / 2 - 1, 0, BTreeUtility.MAX_RAND_VALUE / 2);
        BTreeLeafPage rightPage = BTreeUtility.createRandomLeafPage(rightPageId, 2, keyField, BTreeUtility.getNumTuplesPerPage(2) / 2 - 1, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE);

        // create the parent page and the new entry
        BTreePageId parentId = new BTreePageId(tableid, 1, BTreePageId.INTERNAL);
        BTreeInternalPage parent = BTreeUtility.createRandomInternalPage(parentId, keyField, BTreePageId.LEAF, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE, 2);
    ...
    public void testStealFromLeftInternalPage() throws Exception {
        ...
        BTreeInternalPage page = BTreeUtility.createRandomInternalPage(pageId, keyField, BTreePageId.LEAF, entriesPerPage / 2 - 1, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE, 5 + entriesPerPage);
        BTreeInternalPage sibling = BTreeUtility.createRandomInternalPage(siblingId, keyField, BTreePageId.LEAF, 0, BTreeUtility.MAX_RAND_VALUE / 2, 4);
        ...
        assertTrue(sibling.reverseIterator().next().getKey().compare(Op.LESS_THAN_OR_EQ, page.iterator().next().getKey()));
    ...
    public void testStealFromRightInternalPage() throws Exception {
        ...
        BTreePageId pageId = new BTreePageId(tableid, 1, BTreePageId.INTERNAL);
        BTreePageId siblingId = new BTreePageId(tableid, 2, BTreePageId.INTERNAL);
        BTreeInternalPage page = BTreeUtility.createRandomInternalPage(pageId, keyField, BTreePageId.LEAF, entriesPerPage / 2 - 1, 0, BTreeUtility.MAX_RAND_VALUE / 2, 4);
        BTreeInternalPage sibling = BTreeUtility.createRandomInternalPage(siblingId, keyField, BTreePageId.LEAF, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE, 4 + entriesPerPage / 2);
        ...
        // are the keys in the left page less than the keys in the right page?
        assertTrue(page.reverseIterator().next().getKey().compare(Op.LESS_THAN_OR_EQ, sibling.iterator().next().getKey()));
    ...
    public void testMergeInternalPages() throws Exception {
        ...
        BTreeInternalPage leftPage = BTreeUtility.createRandomInternalPage(leftPageId, keyField, BTreePageId.LEAF, entriesPerPage / 2 - 1, 0, BTreeUtility.MAX_RAND_VALUE / 2, 3 + entriesPerPage);
        BTreeInternalPage rightPage = BTreeUtility.createRandomInternalPage(rightPageId, keyField, BTreePageId.LEAF, entriesPerPage / 2 - 1, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE, 2 + 3 * entriesPerPage / 2);
