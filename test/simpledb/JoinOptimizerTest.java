...
public class JoinOptimizerTest extends SimpleDbTestBase {
    ...
    public void estimateJoinCostTest() throws ParsingException, IOException {
        ...
        JoinOptimizer jo;
        Parser p = new Parser();
        jo = new JoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName1 + " t1, " + tableName2 + " t2 WHERE t1.c1 = t2.c2;"), new ArrayList<>());
        // 1 join 2
        ...
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 1 join 1
        jo = new JoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName1 + " t1, " + tableName1 + " t2 WHERE t1.c3 = t2.c4;"), new ArrayList<>());
        equalsJoinNode = new LogicalJoinNode(tableName1, tableName1, Integer.toString(3), Integer.toString(4), Predicate.Op.EQUALS);
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 2 join 2
        jo = new JoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName2 + " t1, " + tableName2 + " t2 WHERE t1.c8 = t2.c7;"), new ArrayList<>());
    ...
    public void estimateJoinCardinality() throws ParsingException, IOException {
        TransactionId tid = new TransactionId();
        Parser p = new Parser();
        JoinOptimizer j = new JoinOptimizer(p.generateLogicalPlan(tid, "SELECT * FROM " + tableName2 + " t1, " + tableName2 + " t2 WHERE t1.c8 = t2.c7;"), new ArrayList<>());
        ...
        cardinality = j.estimateJoinCardinality(new LogicalJoinNode("t1", "t2", "c" + 3, "c" + 4, Predicate.Op.EQUALS), stats1.estimateTableCardinality(0.8), stats2.estimateTableCardinality(0.2), true, false, TableStats.getStatsMap());
        ...
        cardinality = j.estimateJoinCardinality(new LogicalJoinNode("t1", "t2", "c" + 3, "c" + 4, Predicate.Op.EQUALS), stats1.estimateTableCardinality(0.8), stats2.estimateTableCardinality(0.2), false, true, TableStats.getStatsMap());
    ...
    public void orderJoinsTest() throws ParsingException, IOException {
        ...
        // outermost table.
        Assert.assertFalse(result.get(2).t2Alias.equals("hobbies") && (result.get(0).t1Alias.equals("hobbies") || result.get(0).t2Alias.equals("hobbies")));
    }
    ...
    public void nonequalityOrderJoinsTest() throws IOException, ParsingException {
        ...
        j = new JoinOptimizer(p.generateLogicalPlan(tid, "SELECT COUNT(a.c0) FROM a, b, c, d,e,f,g,h,i WHERE a.c1 < b.c1 AND b.c0 = c.c0 AND c.c1 = d.c1 AND d.c0 = e.c0 AND e.c1 = f.c1 AND f.c0 = g.c0 AND g.c1 = h.c1 AND h.c0 = i.c0;"), nodes);
