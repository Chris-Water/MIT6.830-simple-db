  ...
  public void mergeTupleIntoGroup(Tuple tup) {
    ...
    if (!map.containsKey(gb)) {
      ...
      if (gbfield == Aggregator.NO_GROUPING) {
        ...
        String[] name = {tup.getTupleDesc().getFieldName(gbfield),
            String.format("%s(%s)", op, tup.getTupleDesc().getFieldName(agfield))};
        desc = new TupleDesc(types, name);
        agTuple = new Tuple(desc);
      ...
      switch (op) {
        case MIN:
          oldTuple.setField(idx,
              aggregation.compare(Predicate.Op.LESS_THAN, oldTuple.getField(1)) ? aggregation
                  : oldTuple.getField(1));
          break;
        case MAX:
          oldTuple.setField(idx,
              aggregation.compare(Predicate.Op.GREATER_THAN, oldTuple.getField(1)) ? aggregation
                  : oldTuple.getField(1));
          break;
        case AVG:
