...
public class JoinOptimizer {
  ...
  /**
   * Return best itera0.tor for computing a given logical join, given the specified statistics, and
   ...
   * estimates, and stats only has information about the base tables. For this reason, the plan1 be
   * the inner
   *
   ...
   * the output is, it could be the size of the product of the cardinalities of the tables (if both
   * tables have the same value for all tuples) or it could be 0. It's fine to make up a simple
   * heuristic (say, the size of the larger of the two tables).
   * <p>
   * 3.For range scans, it is similarly hard to say anything accurate about sizes. The size of the
   * output should be proportional to the sizes of the inputs. It is fine to assume that a fixed
   * fraction of the cross-product is emitted by range scans (say, 30%). In general, the cost of a
   * range join should be larger than the cost of a non-primary key equality join of two tables of
   * the same size.
   */
  ...
  /**
   * 例如 v = [0,1,2,3,4]  size = 3 用二进制数0 ~ 2^(v.length) - 1表示每个下标的元素是否选取，对应位取1表示选择对应下标的元素 如
   * [0,1,0,1,1] 从低位到高位表示 子集 [1,3,4] 对应的数字为26
   * <p>
   * 在这个优化的算法中，我们使用一个循环来遍历从 0 到 2^n-1 的所有数字。 这些数字的二进制表示形式刚好可以表示子集中元素的选择情况。 在循环中，我们使用一个内部循环来提取每个数字 i
   * 的二进制位。 我们通过将 i 右移并检查最低位来提取二进制位的值。如果最低位为 1，则将对应位置上的元素添加到子集中。
   * <p>
   * 与位运算的方法相比，这种迭代法的优势在于它不需要进行位运算和计算二进制位数， 而是通过递增地处理数字 i 和列表索引来直接生成子集。这使得算法更加高效且易于理解。
    ...
    for (int i = 1; i <= joins.size(); i++) {
      ...
      for (Set<LogicalJoinNode> subset : subsets) {
        ...
        for (LogicalJoinNode joinToRemove : subset) {
          ...
          CostCard planCost = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove,
              subset, bestPlan.cost, planCache);
          if (planCost == null) {
            continue;
          }
